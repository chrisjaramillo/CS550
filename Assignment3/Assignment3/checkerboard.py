...
class CheckerBoard(Board):
    ...
    # pawns[0] moves towards top of board (row 0)
    # pawns[1] moves towards bottom of board (row N)
    ...
    def __init__(self):
        ...
        # locations per row.
        self.locations_per_row = self.edgesize / self.step
                        
        super(CheckerBoard, self).__init__(self.edgesize, self.edgesize,
                                           displaycol=3)
        
        self.coloffset = [(r + 1) % self.step for r in xrange(self.edgesize)]
        ...
        self.pawnsN = [0, 0]  # Number remaining pawns
        ...
        for row in xrange(self.rows):
            ...
            if row < rowpieces or row >= self.rows - rowpieces:
                if row < rowpieces:
                    ...
                    playeridx = 0
                for col in xrange(self.locations_per_row):
    ...
    def is_terminal(self):
        ...
        piececounts = map(operator.add, self.pawnsN, self.kingsN)
    ...
    def get_actions(self, player):
        ...
        5  r  . <r> . <r> .  .  .    with <> to make it easier
        6  .  r  .  r  .  r  .  r
        7  r  .  r  .  r  .  r  .
        [[(4, 7), (2, 5, (3, 6))], 
         [(5, 2), (3, 4, (4, 3))], 
    ...
    def __movehelper(self, r, c, movepaths, playeridx, history):
        ...
        for m in movepaths:
            ...
            if self.onboard(rmove, cmove):
                ...
                if self.board[rmove][cmove] in self.players[otherplayer]:
                    # Blocked See if capture possible
                    rjump = rmove + m[0]
                    cjump = cmove + m[1]
        ...
        if history and not actions:
            # One or more captures have been made, but we've hit
            # the end of the path.  The history list represents
            # a chain of actions associated with a specific sequence
            # of actions.  As this routine produces a list of valid
            # moves, make it a list
            actions = [history]
